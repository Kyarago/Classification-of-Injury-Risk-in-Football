...
# --------------- findff3_n0c_s (Full data before outlier removal) ---------------
df_full = pd.read_csv('C:/Users/aurim/Desktop/Mokslai/findff3_n0c_s.csv', low_memory=False)
df_full = df_full.drop(['Date', 'Player name', 'Minutes played', 'height', 'dob', 'Player_id',
                        'Injury area condition', 'Injury area_sp condition'], axis=1)

df_full = df_full[df_full['Position'] != 'GK']

df1=df_full[df_full['Injury condition']==1]
df0=df_full[df_full['Injury condition']==0]
del df_full
...
X = X[['Career injuries', 'Player Age', 'Season days injured', '10 Minutes', 'Career days injured',  '30 Minutes',
           'Season Minutes', 'Days without injury', 'Career Minutes']]
...
plt.savefig('C:/Users/aurim/Desktop/Mokslai/Undersampled Data/df full/CH_df_full.pdf', bbox_inches="tight")
plt.close()
...
plt.legend()
plt.show()
plt.savefig('C:/Users/aurim/Desktop/Mokslai/Undersampled Data/df full/PCA_df_full.pdf',
            bbox_inches="tight")
plt.close()
...
# Plot the Calinski-Harabasz scores and save the image
plt.figure()
plt.plot(range(2, 11), calinski_scores, marker='o', color=colors[0])
plt.axvline(x=optimal_k_pca, color=colors[1], linestyle='--', label=f'Optimal k: {optimal_k_pca}')
plt.xlabel('Number of Clusters')
plt.ylabel('Calinski-Harabasz Score')
plt.savefig('C:/Users/aurim/Desktop/Mokslai/Undersampled Data/df full/CH_df_full_pca.pdf', bbox_inches="tight")
plt.close()
...
for label in np.unique(cluster_labels):
    ...
    summary_stats.to_csv(f'C:/Users/aurim/Desktop/Mokslai/Undersampled Data/df full/EXP_cluster_{label}_df_full.csv')

# Boxplots with clusters as classes
for column, y_label, title in zip(columns_to_plot, y_labels, titles):
    ...
    plt.savefig(f'C:/Users/aurim/Desktop/Mokslai/Undersampled Data/df full/BP_Cluster_{column}_df_full.pdf',
                bbox_inches="tight")
    plt.close()
...
print('Proportion to drop:', proportion)
...
df_full_UND = pd.concat([df0_thinned, df1])
df_full_UND.to_csv('C:/Users/aurim/Desktop/Mokslai/Undersampled Data/df full/df_full_UND.csv', index=False)

# Balance of classes in percentage:
print(df_full_UND['Injury condition'].value_counts())
print(df_full_UND['Injury condition'].value_counts(normalize=True) * 100)
